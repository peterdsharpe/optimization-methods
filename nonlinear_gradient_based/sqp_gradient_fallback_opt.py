...
for step in range(max_steps+1):
    # Draw with point
    plt.title(r"SQP (Newton) with 1st-Order Fallback: Iteration %i" % (step))
    ...
    if ddf_func(xs[-1]) > 0:
        xs.append(
            xs[-1] - df_func(xs[-1]) / ddf_func(xs[-1])
        )
    else:
        xs.append(
            xs[-1] - df_func(xs[-1])
...
frames[0].save('sqp_gradient_fallback_%i_iters.gif' % (step), format='GIF',
