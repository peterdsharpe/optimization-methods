...
for step in range(max_steps+1):
    # Draw with point
    plt.title(r"SQP (Newton) w/ Backtracking Line Search: Iteration %i" % (step))
    plt.plot(xs[-1], fs[-1], ".", color=(92/255, 0/255, 0/255), zorder = 4)
    output()

    df_val = df_func(xs[-1])
    ddf_val = ddf_func(xs[-1])
...
frames[0].save('sqp_backtracking_line_search_%i_iters.gif' % (step), format='GIF',
